...
def score_translation(translation):
    ...
    for word in input_words:
        ...
        i = len(translated)
        while i > 0 and translated[:i] not in word_subsets:
            i -= 1
...
def random_translation():
    ...
    return {c: alphabet.pop() for c in string.ascii_lowercase}
...
translation_set = [random_translation() for _ in range(gen_size)]
score_set = [None for _ in range(gen_size)]
generation_count = 0
...
while generation_count != generation_limit:
    ...
    for _ in range(random_surv_count):
